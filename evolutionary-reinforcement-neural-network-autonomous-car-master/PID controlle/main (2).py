...
def optimize_evo(pop_size, gens, epsilon):
    ...
    for cur_gen in range(gens):
        ...
        # fitness = [run_with_params(list(individual), scroll) for individual in population]
        ...
        for _ in range(pop_size - 1):
            ...
            child = ((parent1[0] + parent2[0]) / 2, (parent1[1] + parent2[1]) / 2, (parent1[2] + parent2[2]) / 2)
            ...
            for i in range(len(mutated_individual)):
                if random.random() < mutation_rate and random.random() < 0.5:
                    if random.random() < 0.5:
                        mutated_individual[i] = mutated_individual[i] * 0.5
                    else:
                        mutated_individual[i] = mutated_individual[i] * -0.5
                elif random.random() < mutation_rate:
                    if random.random() < 0.5:
                        mutated_individual[i] = mutated_individual[i] * 2.1
                    else:
                        mutated_individual[i] = mutated_individual[i] * -2.1
    ...
    return params
params = optimize_evo(200, 500, 0.01)
print("----------------------")
print(params)
# creates plot
x = coords[:, 0]
